...
export default function AdminDashboard({ user, data, setData, onLogout, onGraphView, reloadData }) {
	...
	const handleSave = async () => {
		...
		try {
			...
			if (selected.type === "task") {
				...
				await api.put("/api/node/update", {
					node_id: selected.id,
					name: detail.name,
					dept_id: detail.department
				});

				setData((d) => ({
					...d,
					tasks: d.tasks.map((t) =>
						t.id === selected.id ? { ...t, ...detail } : t
					)
				}));

			} else if (selected.type === "department") {

				await api.put("/api/department/update", {
					dept_id: selected.id,
					dept_name: detail.name
				});

				setData((d) => ({
					...d,
					departments: d.departments.map((dep) =>
						dep.id === selected.id ? { ...dep, ...detail } : dep
					)
				}));
	...
	const handleDelete = async () => {
		...
		try {
			...
			if (selected.type === "task") {
				...
				await api.delete("/api/node/delete", {
					data: { node_id: selected.id }
				});

				setData((d) => ({
					...d,
					tasks: d.tasks.filter((t) => t.id !== selected.id)
				}));

			} else if (selected.type === "dependency") {

				await api.post("/api/dependency/delete", {
					id: selected.id
				});

				setData((d) => ({
					...d,
					dependencies: d.dependencies.filter((dep) => dep.id !== selected.id)
				}));

			} else if (selected.type === "department") {

				await api.delete("/api/department/delete", {
					data: { dept_id: selected.id }
				});

				setData((d) => ({
					...d,
					departments: d.departments.filter((dep) => dep.id !== selected.id)
				}));
	...
	const addTask = async () => {
		try {
			const dept = data.departments[0]?.id || 1;
	...
	const addDepartment = async () => {
		try {
			...
			const res = await api.post("/api/department/create", {
				dept_name: "New Dept"
			});

			if (res.status === 201) {

				// backend does not return ID, so reload data
				await reloadData();
